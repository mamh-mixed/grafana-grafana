...
func TestIntegrationDashboardFolderDataAccess(t *testing.T) {
	...
	t.Run("Testing DB", func(t *testing.T) {
		...
		setup := func() {
			...
			dashboardStore = ProvideDashboardStore(sqlStore, &setting.Cfg{}, testFeatureToggles, tagimpl.ProvideService(sqlStore, sqlStore.Cfg), b, quotaService)
		...
		t.Run("Given two dashboard folders with one dashboard each and one dashboard in the root folder", func(t *testing.T) {
			...
			setup2 := func() {
				...
				b := bus.ProvideBus(tracing.InitializeTracerForTest())
				quotaService := quotatest.NewQuotaServiceFake(false, nil)
				dashboardStore := ProvideDashboardStore(sqlStore, sqlStore.Cfg, testFeatureToggles, tagimpl.ProvideService(sqlStore, sqlStore.Cfg), b, quotaService)
