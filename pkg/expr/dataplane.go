...
func handleDataplaneTimeseries(frames data.Frames) (mathexp.Results, error) {
	...
	addNameLabel := shouldUseLabelName(len(sc.Refs), func(i int) data.Labels {
		return sc.Refs[i].GetLabels()
	}, func(i int) string {
		return sc.Refs[i].GetMetricName()
	})

	for _, s := range sc.Refs {
		newSeries, err := mathexp.NewSeriesFromRef(sc.RefID, s)
		if addNameLabel {
			newSeries.SetLabels(data.Labels{
				nameLabelName: s.GetMetricName(),
			})
		}
		if err != nil {
			return res, err
...
func handleDataplaneNumeric(frames data.Frames) (mathexp.Results, error) {
	...
	addNameLabel := shouldUseLabelName(len(nc.Refs), func(i int) data.Labels {
		return nc.Refs[i].GetLabels()
	}, func(i int) string {
		return nc.Refs[i].GetMetricName()
	})
	...
	for _, n := range nc.Refs {
		...
		if addNameLabel {
			newNum.SetLabels(data.Labels{
				nameLabelName: n.GetMetricName(),
			})
		}
		if err != nil {
			return res, err
...
func shouldUseLabelName(count int, getLabels func(int) data.Labels, getName func(int) string) bool {
	for i := 0; i < count; i++ {
		if len(getLabels(i)) > 0 {
			return false
		}
	}
