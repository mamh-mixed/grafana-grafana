...
func TestService_GetHttpTransport(t *testing.T) {
	...
	t.Run("Should use cached proxy", func(t *testing.T) {
		...
		dsService := ProvideService(sqlStore, secretsService, secretsStore, cfg, featuremgmt.WithFeatures(), acmock.New(), acmock.NewMockedPermissionsService(), b, quotaService)
...
func TestService_GetDecryptedValues(t *testing.T) {
	t.Run("should migrate and retrieve values from secure json data", func(t *testing.T) {
		...
		b := bus.ProvideBus(tracing.InitializeTracerForTest())
		quotaService := quotatest.NewQuotaServiceFake(false, nil)
		dsService := ProvideService(sqlStore, secretsService, secretsStore, nil, featuremgmt.WithFeatures(), acmock.New(), acmock.NewMockedPermissionsService(), b, quotaService)
