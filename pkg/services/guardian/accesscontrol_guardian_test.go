...
func setupAccessControlGuardianTest(t *testing.T, uid string, permissions []accesscontrol.Permission, dashboardSvc dashboards.DashboardService) (*AccessControlDashboardGuardian, *models.Dashboard) {
	...
	dashStore := dashdb.ProvideDashboardStore(store, store.Cfg, featuremgmt.WithFeatures(), tagimpl.ProvideService(store, store.Cfg), b, quotaService)
	...
	userSvc := userimpl.ProvideService(store, nil, store.Cfg, nil, nil, b, quotatest.NewQuotaServiceFake(false, nil))
