...
func ProvideDashboardStore(sqlStore db.DB, cfg *setting.Cfg, features featuremgmt.FeatureToggles, tagService tag.Service,
	...
	bus.Publish(context.TODO(), &events.NewQuotaReporter{
		TargetSrv: quota.TargetSrv("dashboard"),
		Reporter:  s.Count,
	})
...
func (d *DashboardStore) Count(ctx context.Context, scopeParams *quota.ScopeParameters) (map[quota.Scope]int64, error) {
	...
	if scopeParams.OrgID != 0 {
		if err := d.store.WithDbSession(ctx, func(sess *sqlstore.DBSession) error {
			rawSQL := fmt.Sprintf("SELECT COUNT(*) AS count FROM dashboard WHERE WHERE org_id=? AND is_folder=%s", d.store.GetDialect().BooleanStr(false))
