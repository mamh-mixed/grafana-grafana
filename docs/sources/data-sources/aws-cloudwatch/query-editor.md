...
- [Logs query editor](#logs-query-editor)
...
It uses a dialect of SQL and this query syntax:
...
The suggestions appear after typing a space, comma, or dollar (`$`) character, or the keyboard combination <key>CTRL+Space</key>.
...
Three fields located at the bottom of the metric query editor are common to both _Metric Search_ and _Metric Query_.
...
> **Alias pattern deprecation:** In Grafana 9, dynamic labels replaced alias patterns in the CloudWatch data source.
...
When making `stats` queries in Explore, make sure you are in Metrics Explore mode.
