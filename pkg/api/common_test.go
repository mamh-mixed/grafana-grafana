...
func setupHTTPServerWithCfgDb(
	...
	quotaService := quotatest.NewQuotaServiceFake(false, nil)
	dashboardsStore := dashboardsstore.ProvideDashboardStore(db, cfg, featuremgmt.WithFeatures(), tagimpl.ProvideService(db, cfg), b, quotaService)
	...
	if useFakeAccessControl {
		...
		b := bus.ProvideBus(tracing.InitializeTracerForTest())
		userSvc = userimpl.ProvideService(db, nil, cfg, teamimpl.ProvideService(db, cfg), localcache.ProvideService(), b, quotatest.NewQuotaServiceFake(false, nil))
	}
